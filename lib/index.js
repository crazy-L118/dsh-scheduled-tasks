...
function startScheduler(ctx) {
  ...
  // Self-heal: a `running` flag persisted in the store is meaningless across
  // host restarts (the in-memory runner always starts idle), and a stale flag
  // makes the scheduler skip the task forever. Clear them all on boot.
  void (async () => {
    try {
      const store = await readStore();
      let changed = false;
      for (const task of store.tasks) {
        if (task.running === true) { task.running = false; changed = true; }
      }
      if (changed) await writeStore(store);
    } catch { /* store issues must not block the scheduler */ }
  })();
